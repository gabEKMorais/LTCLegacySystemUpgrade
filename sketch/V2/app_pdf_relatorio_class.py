...
class app_pdf_relatorio(tk.Frame):
    def __init__(self, master):
        ...
        #   FRAME UNIFICAR
        self.frame_unificar_os = ttk.Frame(self) 
        ...
        self.entry_detalha = tk.Entry(self.frame_unificar_os)
        self.entry_detalha.grid(row=1, column=1, columnspan=3, padx=5, sticky="ew")
        self.frame_unificar_os.grid(row=98, column=0, columnspan=2, padx=5, sticky="ew")
        #   RESTANTE WIDGETS
        self.botao_mostrar_pdf = ttk.Button(self, text="Mostrar PDF", command=lambda: self.exibir_pdf())
        self.botao_mostrar_pdf.grid(row=99, column=0, padx=5, pady=3, sticky="ew") 
        self.botao_gerar_pdf = ttk.Button(self, text="Gerar PDF", command=lambda: self.gerar_pdf())
        self.botao_gerar_pdf.grid(row=99, column=1, padx=5, pady=3, sticky="ew") 

    def carrega_os(self, id_os):
        # objeto com .os , .array_servicos , .array_produtos , label e botao
    ...
    def exibir_pdf(self):
        try:
            pdf_dados = cria_pdf_relatorio(self.os_carregadas, 0, self.unifica_os_var.get(), self.unifica_os_detalha_var.get(), self.entry_detalha.get())
            try:
                    ...
                    for page_num in range(doc.page_count):
                        ...
                        imagens.append(ImageTk.PhotoImage(img))  # Adiciona a imagem à lista
                    # ENVIA A IMAGEM GERADA PARA UMA INSTANCIA DE CLASSE RESPONSAVEL POR MOSTRAR EM TELA
                    app_pdf_view_instance = app_pdf_view(self, imagens)
    ...
    def gerar_pdf(self):
        try:
            cria_pdf_relatorio(self.os_carregadas, 1, self.unifica_os_var.get(), self.unifica_os_detalha_var.get(), self.entry_detalha.get())
        except Exception as e:
            print (f"Erro ao criar PDF: {e}")
